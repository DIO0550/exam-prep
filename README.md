...
- **ビルドだけは 3 回まで試す**（`ci.yml` / `deploy-pages.yml`）。`layout.tsx` の
  ...
  コードやデータの問題と見分けが付かないまま公開が止まるので、前の試行の残り
...
収録しているのは次の 3 系統。どれも 1 つの問題集＝1 ファイルで
...
| 生成AIパスポート 対策（`exam: "GAIP"`） | 用語問題 1 セット（80問）・模擬試験 4 セット（各60問・計240問）・章別 6 セット（各20問・計120問）・GPT特化 1 セット（20問）の計460問 | **本サイトで書き下ろし** |
...
IPA が公開しているのは令和元年度以降なので、**午前については公開ぶんを全部収録している**（平成分は公開されていない）。
令和2年度は春期の実施が取りやめになり、秋の回だけが行われた。IPA がこの回を「令和2年度10月試験」と
呼んでいるので、出典表記も秋期ではなく 10月 にしてある（`Term` の `"oct"`）。
また令和元年度は「令和1年度」ではなく「令和元年度」と書く。
...
Google Cloud 対策の 8 つと生成AIパスポート対策の 12、あわせて 20 の問題集は、
...
**生成AIパスポート対策（`gaip-*.ts`）は、直接読んだ資料と `reference` に書いた資料が一致しない。**
問題を書き起こすときに読んだのは、シラバスの章立てに沿って作られた学習ノートであって、
シラバスや各省庁の資料の原典ではない。`reference` に書いてあるのは、その学習ノートが依拠している
公開資料で、知識の出どころとしてはそちらにあたる。原典と 1 文ずつ突き合わせて検証したわけではないので、
条文の言い回しのような細部は原典で確かめてほしい。各ファイルの冒頭にも同じ断りを書いてある。
...
生成AIパスポート対策（`gaip-*.ts`）は、**同じ範囲を 3 通りの形で解き直せる**ように作り分けてある。
同じ用語でも、問われ方が変わると急に解けなくなるため。

| 問題集 | 問うこと | 出題の形 |
|---|---|---|
| 用語問題1（80問） | 説明文からキーワードを反射で引けるか | 特徴を 3 行ほど並べ、当てはまる用語を 4 択から選ぶ。全 6 章から通しで出す |
| 模擬試験1〜4（各60問） | 本番と同じ形で解けるか | 4 つの選択肢がそれぞれ別のものを説明して誤りを 1 つ選ぶ形と、1 つのものについて特徴を 4 つ並べて 1 つだけ誤りを選ぶ形 |
| 第1章〜第6章（各20問） | 1 章ぶんを総合的に確かめられるか | 上の 2 つに「用語 → 説明を選ぶ」を足した 3 通りを 1 セットに混ぜる |
| 第2章 GPT特化（20問） | GPT と ChatGPT の世代・機能・周辺プロダクトの名前と特徴を取り違えずに引けるか | 系譜の上で隣り合うもの（GPT と GPTs、GPT-4o と o1、o3 の三種、GPT-4.1 と GPT-5 の版の名前）を必ず並べる |

用語問題は 80 問（応用情報の午前 1 回分と同じ）、模擬試験は 60 問（本試験と同じ問題数）にしてある。
章別は 6 章ぶんを各20問で、第1章から順に解けばシラバスを一周できる。
第2章の GPT の話は章別では数問しか入らないので、名前と特徴の対応だけを繰り返せる別セットを置いてある。

**模擬試験 4 セットは、同じ範囲を別の角度から問う。** 同じ形式で 4 回並べても、2 回目からは
見覚えで解けてしまうため、セットごとに問う層をずらしてある。

| セット | 問う層 |
|---|---|
| 模擬試験1 | 各節の幹。定義そのものと、節の中心にある結論 |
| 模擬試験2 | 幹のすぐ隣にある枝。下位項目の顔ぶれや、似た区分の取り違え |
| 模擬試験3 | 「なぜそうなっているか」。理由・背景・目的をすり替えた選択肢が中心 |
| 模擬試験4 | 具体的な場面への当てはめ。業務でありがちな状況を置き、そこでの振る舞いを問う |

どのセットも第1章から第6章まで各10問で、章の配分は揃えてある。

**分野（`field`）の粒度は問題集によって変えてある。** 章別は節の名前（節が 12 以上ある章は
近い節をまとめた 5 つ）、用語問題と模擬試験は章の名前。結果画面の分野別集計は 1 セットの中だけで
集計するので、どのセットでも 4〜6 行に収まるようにそろえた。1〜2 問だけの分野が並ぶと読めなくなるため。
...
本サイトは IPA とも Google とも 生成AI活用普及協会とも無関係の個人制作。Google Cloud 対策と
生成AIパスポート対策の問題は、公式の試験問題でも模擬試験でもない。

## これから

- 午後問題と、他の試験区分（基本情報・情報セキュリティマネジメント）の収録。
  手順は上の表のとおりで、1 問ずつ原本と突き合わせながら足す
- Google Cloud 対策・生成AIパスポート対策の問題の増補と、解説に添える図の追加
