...
単語帳（`apps/web/src/features/vocab`）は、設定 → めくる → 結果の 3 画面。

| 画面 | 中身 |
|---|---|
| 設定 | 出題形式・分野・順番・枚数と、「あやふや」だけに絞るかを決めて始める |
| めくる | 1 枚ずつ表と裏。上の帯に進み具合と覚えた／あやふやの数、下に札番号のボタンを出す |
| 結果 | 覚えた割合と、あやふやだった語の一覧。同じ札・あやふやだけ・設定に戻るの 3 つから選ぶ |

**「今どこにいるか」は演習と同じ形で出す。** 上の帯（`ProgressBar`）は両方で共有していて、
数の単位（「問」「枚」）と、右のチップに出す成績（演習は正答率、単語帳は覚えた／あやふや）
だけを呼ぶ側から渡す。下の番号ボタンは、演習が `QuestionDots`、単語帳が `CardDots` と
別のコンポーネントだが、見た目と操作は同じ（押せばその問題・その札へ直接移る）。
画面をまたぐたびに置き場所と形が変わると、そのつど読み直すことになるため。

**めくっている最中でも「設定に戻る」で抜けられる。** 以前は Esc キーしか無く、
触って使う端末では最後までめくるしかなかった。
...
- **落とすのは 1 文字も無くなったときだけ**。空白や改行だけでも持っておく（「空白だけなら空」と
  すると、書き始めに改行を打った時点でメモごと消えて、打った改行が戻ってこない）。
  「メモあり」の印のほうは、読めるものが無ければ出さない
...
収録しているのは次の 2 系統。どちらも 1 つの問題集＝1 ファイルで
`apps/web/src/features/quiz/data/` に置き、
[`data/questions.ts`](apps/web/src/features/quiz/data/questions.ts) の `QUESTION_SETS` に並べる。

| 系統 | 中身 | 出どころ |
|---|---|---|
| IPA 過去問題（`exam: "AP"`） | 応用情報技術者試験 午前の10回分（令和3年度 春期〜令和7年度 秋期、各80問・計800問） | 原本は IPA の公開 PDF |
| Google Cloud 対策（`exam: "CDL"`） | Cloud Digital Leader 向けのシナリオ問題 4 セットとサービス確認問題 4 セット（各30問・計240問） | **本サイトで書き下ろし** |

**左の一覧で選んだ試験と「出題する回」は連動する。** `QuestionSet` の `exam` が
[`data/exams.ts`](apps/web/src/features/quiz/data/exams.ts) の `code` と対応していて、
ヘッダーの「出題する回」にはその試験の問題集だけが並ぶ。選んでいる試験は出題中の問題集から
引いていて、別の state に持たない（持つと、記録から読み直したときに左の選択とずれる）。
切り替えると学習ホームに戻る。解答状況は問題ごとに残るので、戻ってくれば続きから解ける。
...
Google Cloud 対策の 8 つの問題集は、**問題文・選択肢・正解・解説のすべてを本リポジトリで書いた**もの。
どこかの認定試験の実際の設問ではなく、公式の模擬試験でもない。

原本が無いので年度や問番号では指せない。代わりに `Source` のもう 1 つの形
（`kind: "original"`）を使い、「どの問題集の何問目か」（`deck` / `label` / `no`）と
**何を典拠に書いたか**（`reference`）を持たせる。出典表記は
`GCP シナリオ1 問7・本サイト作成（典拠 …）` の形になり、典拠が空でないことは
[`questions.test.ts`](apps/web/src/features/quiz/data/questions.test.ts) で検査する。
出どころの分からない問題を混ぜないため、`reference` は省略できない。

典拠にしたのは次の公開資料。

- Google Cloud「Cloud Digital Leader 学習ガイド v2.0」（6 分野のモジュール構成・用語集・プロダクト一覧）

**出題範囲は学習ガイド v2.0 の 6 分野に限る。** ガイドの 6 分野・用語集・プロダクト一覧に
出てこない枠組み（Google Cloud Adoption Framework の成熟度フェーズやテーマなど）は、
実際の出題と外れるため問題にしない。
...
| 問題集 | 問うこと | 選択肢の組み立て |
|---|---|---|
| シナリオ問題 1〜4（各30問） | 立場と制約のある場面を読み、選び方そのものを決められるか | 1 段ずれた粒度（ゾーンとリージョン、再ホストとリプラットフォーム）や、要件に対して過剰な構成を混ぜる |
| サービス確認問題 1〜4（各30問） | プロダクトと用語の名前・役割を素早く引けるか | 役割が隣り合うプロダクト（Cloud Run と Cloud Functions、BigQuery と Bigtable、Cloud Trace と Cloud Profiler など）や、対になる用語（予算と割り当て、SLI と SLO と SLA）を必ず置く |

シナリオは 1 セット 30 問を **6 分野に 5 問ずつ配って**組んである。どのセットを解いても
分野の偏りが出ないようにするため。サービス確認は、1 がプロダクト一覧、2 が用語集のコース1・2、
3 がコース3・4 と移行の用語とストレージクラス、4 がコース5・6 を扱う。
...
上とは別に、**本サイトで書いた解説にも図を付けられる**（`Question` の `figure`）。型は 7 つで、
`FigureBlock` がそれぞれの見た目を持つ。`figure` に配列を渡せば、1 問に 2 つ以上並べられる。

**基本は図。表はその補助**にする。「どの組合せなら間に合うか」「どの図はどれか」のような設問は、
数字や言葉を表に並べるより、**同じ軸の上に選択肢を並べて見比べられる絵**にしたほうが速い。
表を足すのは、絵で分かったことを数で確かめたいとき（計算の内訳など）と、用語を並べるだけで
足りる簡単なときに限る。順番は図 → 表。
...
| `timeline` | 時間軸の上に帯を置く。`groups` で選択肢ごとに積める | 多重度・スケジューリング・周期タスクなど、**時刻と重なり**が要点のもの |
| `sketch` | 図の形の見本を並べる（20 種類の SVG） | 連関図・クラス図・B⁺木など、**その図がどんな形か**を知らないと選べないもの |
| `tree` | 2 分木。節点は配列表現の添字で持つ | 2 分探索木の削除、配列での木の表現など、**親子のつながり**が要点のもの |
...
**選択肢を見比べる設問では、`timeline` の `groups` を使う**（例: 令和5年度 秋期 問17）。
まとまりごとに見出し（`label` は記号、`note` は条件と結論、`verdict` は ○×の色）と、
締切の縦線（`deadline`）、締切までに終わらなかった分（`missed`）を置ける。凡例は自動で付く。
ア〜エを同じ時間軸に積むと、「どこで締切をまたぐか」を数えずに見て決められる。

`tree` は節点を配列表現の添字（根が 1、左の子が 2i、右の子が 2i+1）で受け取る。親子の線を
データに書かせると、書き間違いがそのまま木の形になってしまうため。横位置は居る節点だけを
通りがけ順に並べて詰めるので、片側だけ深い木でも間延びしない。
...
- **Google Cloud 対策の問題（`gcp-cdl-*.ts`）**: 本リポジトリで書き下ろしたもの（MIT）。
  公開資料を典拠にしているが、問題文・選択肢・正解・解説はいずれも本リポジトリの著作物で、
  Google の資料からの転載ではない

本サイトは IPA とも Google とも無関係の個人制作。Google Cloud 対策の問題は公式の試験問題でも
模擬試験でもない。
...
- Google Cloud 対策の問題の増補と、解説に添える図の追加
