...
| 生成AIパスポート 対策（`exam: "GAIP"`） | 用語問題 1 セット（80問）・模擬試験 1 セット（60問）・章別 6 セット（各20問・計120問）の計260問 | **本サイトで書き下ろし** |
...
Google Cloud 対策の 8 つと生成AIパスポート対策の 8 つ、あわせて 16 の問題集は、
...
| 模擬試験1（60問） | 本番と同じ形で解けるか | 4 つの選択肢がそれぞれ別のものを説明して誤りを 1 つ選ぶ形と、1 つのものについて特徴を 4 つ並べて 1 つだけ誤りを選ぶ形 |
