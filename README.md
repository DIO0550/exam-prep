...
| Google Cloud 対策（`exam: "CDL"`） | Cloud Digital Leader 向けのシナリオ問題30問とサービス確認問題30問 | **本サイトで書き下ろし** |
...
Google Cloud 対策の 2 つの問題集は、**問題文・選択肢・正解・解説のすべてを本リポジトリで書いた**もの。
...
`GCP シナリオ 問7・本サイト作成（典拠 …）` の形になり、典拠が空でないことは
...
| 問題集 | 問うこと | 選択肢の組み立て |
|---|---|---|
| シナリオ問題（30問） | 立場と制約のある場面を読み、選び方そのものを決められるか | 正解の 1 段手前・1 段先の成熟度フェーズや、要件に対して過剰な構成を混ぜる |
| サービス確認問題（30問） | プロダクトの名前と役割を素早く引けるか | 役割が隣り合うプロダクト（Cloud Run と Cloud Functions、BigQuery と Bigtable、Cloud Trace と Cloud Profiler など）を必ず置く |
