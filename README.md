...
| 演習 | 問題・選択肢・正誤・解説。解説は「同画面」「別画面」、選択肢は「原本順」「シャッフル」、文字は「標準」「大」「特大」を切り替えられる。右にメモを開ける |
...
| `sequence` | 登場人物を横に並べ、間を番号付きの矢印で結ぶ | 認証やプロトコルなど、**誰が誰に何を送るか**が要点のもの |
| `quadrant` | 2 本の軸で 4 つに分けて置く | PPM やアンゾフなど、**2 軸のどこに位置するか**が要点のもの |
| `layers` | 積み重なった層を描く | OSI や 3 層スキーマなど、**上下の関係**が要点のもの |
| `tree` | 節と枝を SVG で描く | 2 分探索木や式木など、**どこにぶら下がっているか**が要点のもの |

**`table` は「表」であって「図」ではない。** 似た用語を見比べるには表が要るが、順番・位置・
構造が要点のものは、下の 4 つのように実際に線と矢印で描いたほうが速く読める。表で
「高 × 低」と書くくらいなら 4 象限に置く、「A が B に送る」と書くくらいなら矢印を引く。
...
足すと、軸の下に目印が立つ。`sequence` は列の中心から中心へ矢印を引き、`reply` で戻りを破線に、
`from` と `to` が同じものはその場での処理として箱で置く。`tree` は葉を左から並べて親を子の
真ん中に置く（座標の決め方は [`tree-layout.ts`](apps/web/src/features/quiz/components/tree-layout.ts)
に分けてあり、テストで固定してある）。

いずれも幅は em で持たせてあるので、文字サイズの設定でそのまま拡大される。狭い画面では
横スクロールさせる（幅に合わせて詰めると、段の対応や時間の長さや矢印の起点が壊れるため）。
